...
// src/Demo.ArmA.WasmWorker/wwwroot/duckdb-gateway.src.js
var db = null;
var conn = null;
var parked = /* @__PURE__ */ new Map();
...
async function attachDataset(url, name) {
...
async function runQuery(sql, handleId) {
  ...
  try {
    ...
    const bytes = batches.length ? tableToIPC(new Table(batches), "stream") : new Uint8Array(0);
...
function copyResult(handleId, memoryView) {
...
async function cancelQuery(handleId) {
  const c = inFlight.get(handleId);
  if (!c) return false;
  try {
    return await c.cancelSent();
  } catch {
    return false;
  }
}
